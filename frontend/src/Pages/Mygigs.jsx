  ...
  return (
    <div className="w-[90%] mx-auto -mt-16 sm:mt-0">
      <div className="font-bold flex justify-between text-4xl my-10 text-blue-900 dark:text-white">
        ...
        {data?.length > 0 && (
          <button
            className="bg-blue-900 hover:bg-blue-800 text-white rounded-xl py-2 px-3 text-sm font-medium"
            onClick={() => navigate("/addgigs")}
          >
            Create New Gig
          </button>
        )}
      ...
      {!isFetching && data?.length === 0 && (
        <div className="font-bold text-3xl flex flex-col gap-5 text-center justify-center items-center h-[40vh] text-red-700">
          ...
          <button
            className="bg-blue-900 hover:bg-blue-800 text-white rounded-xl p-2"
            onClick={() => navigate("/addgigs")}
          >
            Create New Gig
          </button>
          ...
          data?.map((g) => (
            ...
            >
              ...
              <div className="flex justify-between items-center space-x-2 mt-4">
                <Link
                  to={`/gig/${g._id}`}
                  className="bg-blue-900 text-white py-2 px-3 text-sm rounded-md transition duration-300 hover:bg-blue-700"
                >
                  View
                </Link>
                <Link
                  to={`/editgig/${g._id}`}
                  className="bg-green-600 text-white py-2 px-3 text-sm rounded-md transition duration-300 hover:bg-green-700"
                >
                  Edit
                </Link>
                <button
                  onClick={() => handleDelete(g._id)}
                  className="bg-red-900 text-white py-2 px-3 text-sm rounded-md transition duration-300 hover:bg-red-700"
                >
                  Delete
                </button>
              </div>
